...
# processor. Nothing is persisted, so every invocation is a fresh sign-in.
#
...
#                         1 to authenticate the client on the code exchange.
#                         Needed only when the redirect URI is registered
#                         under the "Web" platform. Default 0.
...
# must use the GUID form or Azure returns AADSTS90009. No offline_access: with
# nothing persisted between invocations a refresh token could never be used.
...
emit() {
    local access
    access=$(printf '%s' "$1" | json_str access_token)
    [[ -n "$access" ]] || return 1
    printf '%s\n' "$access"
}
